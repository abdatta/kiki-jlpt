...
- [x] 6.3 Build the two node species on top of the existing `.workflowNode` conventions: LLM-call cards (eyebrow call-kind, status-inflected title, one-line output stat, model + duration meta line) and compact code-background deterministic nodes; keep status-class-equals-status-string so existing recipes apply; skipped steps render dashed with explanatory stats, never removed
