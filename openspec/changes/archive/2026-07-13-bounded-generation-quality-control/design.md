...
- **Node anatomy:** status icon + status-inflected bold title (the single header — no separate kind eyebrow), then the compact **output stat** (the load-bearing element). Most nodes show one line — e.g. generation `20 conversations · 3 with OOV`; triage `14 pass · 5 repair · 1 regen`; repair candidate `5 convos · OOV 7→2`; re-roll `2 replacements · 1 dropped for good`; final audit `38/40 accepted · coverage 96% · PASS`. The version-comparing nodes show a per-version stack derived at render time from their persisted per-conversation facts (never stored as display text) — gates `Repair N: x eliminated · y coverage-loss` per candidate; pick `Original / Repair 1 / Repair 2: n picked · n good · n okay`. LLM nodes add the model/duration meta line (beside the title on stacked nodes).
